...
FROM
	(
		...
		WHERE
			...
			(SD1010.D1_FILIAL= @filial AND SD1010.D1_LOCAL= @armazem) --LOCAL E FILIAL
		GROUP BY
			SD1010.D1_FILIAL,
			SD1010.D1_LOCAL,
		...
		WHERE
			...
			(SD1010.D1_FILIAL= @filial AND SD1010.D1_LOCAL= @armazem)
		GROUP BY
			SD1010.D1_FILIAL,
			SD1010.D1_LOCAL,
			CASE WHEN Z01010.Z01_TME = '001' THEN 'Endicon' WHEN Z01010.Z01_TME = '002' THEN 'Cliente' WHEN Z01010.Z01_TME = '006' THEN 'Reuso' ELSE 'ERRO' END,
...
SELECT
			 ...
			 #movimentos.CODIGO,
			 #movimentos.DT,
			 SUM(CASE WHEN #movimentos.MOVIMENTO='ENTRADA' THEN 1 ELSE 0 END) AS TRANSACT_ENTRA, --CONTA ENTRADA
			 SUM(CASE WHEN #movimentos.MOVIMENTO='SAIDA' THEN 1 ELSE 0 END) AS TRANSACT_SAI, --CONTA SAÍDA
			 SUM(CASE WHEN #movimentos.MOVIMENTO ='ENTRADA' THEN #movimentos.QUANTIDADE ELSE 0 END) AS SOMA_ENTRA, --SOMA ENTRADA
			 SUM(CASE WHEN #movimentos.MOVIMENTO ='SAIDA' THEN #movimentos.QUANTIDADE ELSE 0 END) AS SOMA_SAI --SOMA SAÍDA
FROM #movimentos
GROUP BY
				 #movimentos.FILIAL,
				 #movimentos.LOCAL,
